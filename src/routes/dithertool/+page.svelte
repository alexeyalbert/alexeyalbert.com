<script lang="ts">
    ...
    for (let y = 0; y < grainTileSize; y++) {
      for (let x = 0; x < grainTileSize; x++) {
        ...
        if (grainMonochrome) {
          ...
          d[i] = Math.max(
            0,
            Math.min(255, v + Math.round((Math.random() - 0.5) * 20)),
          );
          d[i + 1] = Math.max(
            0,
            Math.min(255, v + Math.round((Math.random() - 0.5) * 20)),
    ...
    switch (ditheringEnabled ? ditherMethod : "none") {
      ...
      case "bayer":
        for (let i = 0; i < data.length; i += 4) {
          ...
          let grayscale = rgbToGrayscale(data[i], data[i + 1], data[i + 2]);
          grayscale = adjustPixel(grayscale, exposure, brightness, contrast);
          const useSecondary = bayerDither(
            grayscale,
            x,
            y,
            bayerSize,
  ...
  function applyOverlayToContext(
  ...
  function buildDownloadFilename(): string {
    ...
    const methodPart = (() => {
      ...
      if (ditherMethod === "bayer")
        return `bayer-${bayerSize}x${bayerSize}-amt${fmt(bayerAmount)}`;
      return ditherMethod;
    })();
    ...
    const baseName = uploadedFileName
      ? safe(stripExt(uploadedFileName)) + "_"
      : "";
...
<main>
  <h1>DitherTool</h1>
  <p>
    Upload an image and apply various dithering algorithms or a tinted grain
    overlay
  </p>
    ...
    <div class="controls">
      <div class="control-group">
        ...
        >
          <div class="dropzone-text">
            {uploadedFileName || "Choose image or drop file here"}
          </div>
        </div>
      </div>
      ...
      <div class="control-group">
        <button
          ...
          class:is-on={ditheringEnabled}
          on:click={toggleDithering}
        >
      ...
      {#if ditheringEnabled}
        <div class="warning">
          ...
          <span>
            Dithered previews may not render exactly in the browser. Use the
            downloaded image for the most accurate result.
          </span>
        </div>
      {/if}
      ...
      {#if ditheringEnabled}
        ...
        {#if ditherMethod === "bayer"}
          <div class="control-group">
            <label for="bayer-size"
              >Bayer Matrix Size: {bayerSize}×{bayerSize}</label
            >
            <input
              id="bayer-size"
              type="range"
          ...
          <div class="control-group">
            <label for="bayer-amount"
              >Bayer Amount: {bayerAmount.toFixed(2)}</label
            >
            <input
              id="bayer-amount"
              type="range"
      ...
      {#if overlayEnabled}
        ...
        <div class="control-group">
          <label for="overlay-opacity"
            >Overlay Opacity: {overlayOpacity.toFixed(2)}</label
          >
          <input
            id="overlay-opacity"
            type="range"
        ...
        {#if overlaySource === "generated"}
          <div class="control-group">
            <label for="grain-scale"
              >Grain Scale: {grainScale.toFixed(2)}×</label
            >
            <input
              id="grain-scale"
              type="range"
          ...
          <div class="control-group">
            <label for="grain-intensity"
              >Grain Intensity: {grainIntensity.toFixed(2)}</label
            >
            <input
              id="grain-intensity"
              type="range"
              ...
              max="2"
              step="0.05"
              bind:value={grainIntensity}
            />
          </div>
          <div class="control-group">
            <label>
              <input type="checkbox" bind:checked={grainMonochrome} /> Monochrome
              Grain
            </label>
          </div>
        {/if}
        ...
        <div class="control-group">
          <label for="base-desaturate"
            >Base Desaturate: {baseDesaturate.toFixed(2)}</label
          >
          <input
            id="base-desaturate"
            type="range"
        ...
        <div class="control-group">
          <label for="overlay-tint-opacity"
            >Tint Opacity: {overlayTintOpacity.toFixed(2)}</label
          >
          <input
      ...
      <div class="control-group">
        <label for="scale"
          >Scale: {scale.toFixed(2)}x ({Math.round(
            originalImage?.width * scale || 0,
          )}×{Math.round(originalImage?.height * scale || 0)})</label
        >
        <input
          id="scale"
          type="range"
      ...
      <div class="canvas-wrapper">
        ...
        {#if originalImage}
          <div class="resolution-display">
            Resolution: {Math.round(originalImage.width * scale)}×{Math.round(
